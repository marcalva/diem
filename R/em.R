...
#' @export
run_mv_em_diag <- function(x, k=2, min_iter=5, max_iter=1000, labels=NULL, eps=1e-10, seedn=NULL, verbose=TRUE){
	# Mixture of multivariate normals.
	# Need to estimate parameters for the 2 models, and tau (mixing parameter)
	# Then we can estimate the cluster assignemts
	# 1) Initialize parameters to random values.
	# 2) Compute the probabilities of the latent variable assignments
	# 3) Compute a better estimate of the parameters theta
	...
	# TODO give option to set seed for runif
	set.seed(seedn)
	params <- init_param(x, k)
	mu <- params$mu
	sgma <- params$sgma
	tau <- params$tau
	llk <- -Inf
	...
	for (iter in 1:max_iter){
		...
		if (any(tau_n == 0)) stop("No membership in 1 group. Either badly initialized values or a bug")
		
		# Evaluate data likelihood 
		llk_n <- get_llkCPP(x, mu_n, sgma_n, tau_n, semisup=semisup, labels);
		if (semisup){
			# llk_n <- get_llk_ss(x, mu_n, sgma_n, tau_n, labels)
		} else {
			# llk_n <- get_llk(x, mu_n, sgma_n, tau_n)
		}
